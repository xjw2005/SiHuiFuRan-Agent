"""
Reflection Pattern Demo — LangGraph 实现

核心流程：
  Generate（生成初版答案）→ Reflect（自我反思/评判）→
    ├─ 不够好 → Revise（根据反思修改）→ Reflect → ...
    └─ 足够好 → END（输出最终答案）

这是 LangGraph 官方 Reflection 模式的复现：
...
from langchain_openai import ChatOpenAI  # pyright: ignore[reportAttributeAccessIssue]
...
class ReflectionState(TypedDict):
    """Reflection 流程的状态，messages 用 add_messages reducer 自动累积"""
    messages: Annotated[list, add_messages]
    task: str                # 用户原始任务
    iteration: int           # 当前迭代次数
    max_iterations: int      # 最大反思轮次
    is_satisfactory: bool    # 反思后是否满意
...
def build_reflection_graph() -> CompiledStateGraph:
    ...
    # reflect → conditional: continue/end
