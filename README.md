...
当前先引入 Harness Engineering 的两块能力：human-in-the-loop approval 和 checkpoint/resume。`BashTool` 被包成更像真实 harness 的执行层，CLI 运行时则持续保存可恢复的工作快照。这里参考的是 Claude Code 公开文档里可见的机制：命令权限/审批、默认和最大 timeout、fresh shell 执行、环境注入、长输出处理、后台任务、可恢复工作状态。Claude Code 源码和内部提示未公开，因此这里不照搬源码，而是实现同类边界能力。
...
checkpoint 文件都位于当前 workspace 的 `.mokioclaw/checkpoints/`，内部 git repo 只用于该 workspace 文件快照，不会接管项目仓库。trace 观测和日志落盘仍留到后续 Harness Engineering 继续补齐。
...
14. 如果 verifier 通过，进入 `final`，输出文件、来源、验收结论、压缩次数和运行方式；CLI 会直接展示这个最终总结。
