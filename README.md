...
| `BashTool` | 执行开发命令 | 固定 workspace、超时、输出截断、基础安全拦截；允许 `2>/dev/null` 这类静默检查；高风险命令需要人类审批 |

## Harness Engineering

当前先引入 Harness Engineering 的第一块：human-in-the-loop approval。`BashTool` 会在执行高风险命令前暂停，交给 CLI 中的人类操作者确认。
